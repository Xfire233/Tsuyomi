...
- Returning from the reader no longer reloads the whole detail page. Entering the reader removed the detail destination from composition, which disposed its route owner through `DisposableEffect` and cancelled the load that owner had started; returning then found a loading state nothing was driving and restarted the entire load, so a quick reader round trip cost a full reload. The owner is now held by the navigation entry (`ViewModelProvider(entry, ...)`, the same pattern as the search owner holder) and reloads only when the held content belongs to a different book. Verified on the maintainer's phone: the return now enters with no load requested.
- A challenged network can now use the verified browser transport at all, and pays far less for it. The update probe opened its client with the native-only gateway factory, so the verified transport was always null and a challenged check could never retry on it; the probe now pairs both gateways. The transport reused neither its WebView nor its session: it built and destroyed one per request and polled for readiness, and it failed outright when the encrypted partition held no session. It now keeps one warm WebView, settles from the page-finished callback, adopts the browsing WebView's admitted cookie jar for a declared origin when the partition is empty, paces only after a burst exceeds what one page load needs, backs off when the site rejects a burst, and releases the warm view when the app leaves the foreground. Measured on the challenged network: 334-1014ms per fetch, 558-906ms for the first fetch including view creation, against 2.3-3.8s per fetch under the earlier fixed four-second cadence.
- Credential-bound caches are no longer orphaned by a credential refresh. The cache partition revision was the digest of the whole encrypted credential record, whose random initialisation vector changes on every write, so the verified transport's post-fetch cookie rewrite minted a new partition each time and every cached page became unreachable. ADR 0019 records the decision: the record now carries a random partition id bound into the AEAD associated data, reused on later writes and renewed only when the session's admitted cookie-name set changes or the partition is deleted, with version 1 records still readable and upgraded on their next write.
- A cached page that fails source classification is now evicted instead of being served again, and the split between cacheable and non-cacheable source reads is stated in `docs/architecture/SOURCES.md` rather than left to each source. Every read in the Wenku8 source declared a network-only mode, so the host cache could be written but never read; search, home, detail, directory and chapter now declare cache-first while the update check and the remote-library operations keep network-only (`org.tsuyomi.wenku8-v0.2.37`).
