...
object ReviewNodeCatalog {
    ...
    val nodes: List<ReviewNode> = listOf(
        ...
        node("L08", "网站镜像", ReviewNodeFamily.LIBRARY, AtlasRoute.LIBRARY_MIRROR,
            states(AtlasPageState.LOADING, AtlasPageState.CONTENT, AtlasPageState.EMPTY, AtlasPageState.ERROR, AtlasPageState.OFFLINE, AtlasPageState.SELECTION, AtlasPageState.MUTATION, AtlasPageState.UNRESOLVED, AtlasPageState.MODAL),
            "从 Library 根固定镜像根并从 Browse 来源主页进入同一路由；验证新来源默认只显示聚合的全部网站收藏", "从镜像更多菜单使用完整文字固定/移出快捷书架，再按来源启用网站分组；验证文件夹节点、独立 folder pin、专页与排序；关闭后隐藏但保留 pin/顺序/membership，再启用恢复；有既存 folder pin 的迁移自动保留分组模式，仅有 target 数据不自动开启", "简单模式把本地单本拖到镜像根并直接 ADD 至网站默认目标，且不出现 target picker/MOVE；两种模式都验证本地复制和确认移除；分组模式再验证根 target picker、精确文件夹 ADD→MOVE 与远端文件夹间 MOVE；多选批次尝试网站目标并验证拒绝原因", "验证 ADD/MOVE 首次分别授权、后续精确动作不重复确认但仍生成新 token/reconciliation；MOVE 入口只在分组模式出现；制造 ADD 已确认且 MOVE unresolved 后只重试 MOVE或保留默认组；刷新后让已固定 targetId 消失并验证冻结修复入口", "从无封面的网站收藏进入 canonical Detail 获取封面后返回；镜像必须立即显示该封面，随后再次刷新缺少封面的远端列表也不得退回占位图",
            visual = listOf("默认简单模式完全复用 Library 的书籍 grid/list/compact 卡片、封面、间距、选择态和方向 FAB，只显示全部网站收藏且无 folder/picker/MOVE chrome", "镜像更多菜单以完整动词清楚提供固定/移出快捷书架和按来源启用/停用网站分组；镜像管理从不以图标-only + 或垃圾桶表达，切换也不声称或触发远端合并", "分组模式的镜像根先显示远端文件夹节点再显示全部远端书籍；文件夹子页有自己的 AppBar、刷新、图钉、布局和排序，且不递归显示 Library 根快捷栏", "镜像根和分组模式文件夹快捷项保持 Atlas 80×116dp 静止结构；独立固定且同来源相邻插入；停用时隐藏保留，失效目标冻结而非静默消失", "MOVE、本地复制、网站移除三个拖放目标使用不同文案、语义和视觉层级；简单模式不显示 MOVE；网站移除始终进入明确确认", "确认成功立即更新镜像本地投影和快捷计数；部分完成显示书籍位于默认网站书架并提供继续移动/保留默认书架"),
            human = listOf("默认入口是否像一个普通网站书架而非分组管理器", "未使用网站分组的用户是否无需理解目标分类", "启用分组后的三种拖放结果是否可预测", "切换分组是否可信地保留用户布局和网站数据", "持久授权后的直接 ADD/MOVE 是否仍有足够目标反馈", "远端目标失效与部分完成是否可信可恢复")),
        node("B01", "书籍详情", ReviewNodeFamily.BOOK_READER, AtlasRoute.BOOK_DETAIL,
            ...
            "验证 135×180dp 封面、最多两行的默认 Title-role 书名、仅真实第三行溢出时的圆形 Material Info 图标和完整书名原位展开/收起；封面右栏按标题一/二行、独立作者行、状态+来源更新行、评分、SplitButton 的顺序铺满同高区域；不得出现 Badge/chip 或把作者合并进状态日期行；旧扩展缺失字段时不伪造日期", "点击作者后立即显示同来源作者搜索结果，不再点搜索；进入结果详情再 Back 不重复请求，普通草稿恢复仍不查询", "验证官方 Material 3 SplitButton 主段直接加入本地根，入库后禁用；副段始终打开锚定下拉菜单而非 Sheet", "下拉菜单分别操作稍后再读、快捷书架、本地收藏夹和简单/分组网站目标；保持本地与远端边界、精确目标与部分 MOVE 恢复", "验证长简介三行预览与展开/收起，以及分卷展开、真实未读过滤、独立排序、精确章节 Reader 导航和跨入口 canonical Detail",
            "在 1080×2400@420、字体 1.0 的常用竖屏，只按书名本身判断两行溢出；刚好两行完整显示且无 Info。作者独占标题后的下一行；状态和日期同用下一行；评分和 SplitButton 依次纵向排列。首块贴右栏顶部、按钮贴封面底部，四个块间剩余高度均匀分配，不得集中为空白区域",
            "评分使用无外框的 36dp 布局带，星形 20dp、中心距 28dp；第一颗可见星形的左边缘必须与标题、作者、状态日期和 SplitButton 外框左边缘在 1px 内对齐，不能只对齐不可见布局带。不得有 Surface 填充、描边、胶囊、分隔线或阴影。五个键盘/TalkBack 动作、选中/禁用、相邻边界选择与重复清除不变",
            "将同一 Standard 手机转为横屏，按实际系统栏与 cutout 后的安全宽度检查完整 split 副段和阅读 FAB；不得被侧边系统导航栏遮挡，也不得重复消费 inset",
            "验证 Detail 的权威 summary 升级同一 BookIdentity 的 Library 元数据；加入书架、稍后再读和后续进度保存均保留 cover/author/canonical 字段",
            "日期只来自归一化源站更新字段；来源或旧扩展缺失时不显示占位日期，升级扩展并刷新后恢复；不动持久阅读进度、继续阅读或目录当前章节标记；作者链接支持键盘/TalkBack，失败不得静默回退书名搜索",
            visual = listOf("标题一/二行、作者独立行、状态+日期行、评分、SplitButton 从上到下占满封面右栏；按钮底与封面底对齐，剩余高度均分而非留在某一处", "状态与日期共享普通 metadata 行且无 Badge/chip；作者独占上一行、无下划线并使用低饱和蓝色", "评分区域没有外框、填充、胶囊、分隔线或阴影，只保留五颗星；第一颗可见星形与标题/作者/状态日期/SplitButton 左缘严格对齐，评分与按钮不视觉粘连", "刚好两行书名完整显示且无 Info；真实第三行溢出才显示文字同尺度圆形 i，展开/收起不遮挡书名", "下拉菜单始终锚定在按钮下方，不从屏幕底部弹出；稍后再读、快捷书架、手动收藏夹、网站目标与恢复动作具有完整标签和状态", "正文不再显示独立稍后再读按钮；标签区仅实际标签与行内加号", "简介三行预览与贴尾展开保持紧凑；分卷目录保留三角 disclosure、真实未读过滤、独立排序与当前/下载标记", "移出书架与网站移除仍在 overflow，MOVE/分组目标仅分组模式出现；FAB 不遮挡内容，无本地详情中间页"),
            human = listOf("六个视觉行是否自然填满封面右侧而不显拥挤或留底部空洞", "SplitButton 主/副动作是否无需试错即可预测", "本地多位置与网站单目标是否边界清楚", "上方身份区信息密度是否平衡", "三行简介是否足够且展开动作是否自然", "主操作优先级与页面密度", "数百章按分卷扫描效率", "卷展开与目录排序能否无需试错即区分", "评分可发现性", "章节状态与排序方向能否一眼识别", "二级动作是否无重复且作用域明确")),
        ...
        node("S02", "聚合搜索", ReviewNodeFamily.SOURCE, AtlasRoute.SEARCH,
            ...
            "输入但不提交并确认结果不变", "点击唯一 Search 并观察本地与真实线上结果增量", "从详情作者链接直接提交一次同来源作者搜索并应用结果；结果详情再返回仍保留结果且零重放；验证作者模式的手动验证、离线和显式重试", "制造线上单来源失败、去重、空态和离线；再用签名 fixture 重放",
