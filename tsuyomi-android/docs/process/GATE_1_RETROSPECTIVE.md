...
Gate 1 的功能目标已达到，但过程中暴露出一个共同缺陷：若约束只存在于设计文档或单个测试中，它会在相邻层、另一 profile、另一设备矩阵或另一工具链上再次失效。进入 Gate 2 前，所有已知问题必须形成“源头修复 + 自动验证 + 流程准入”三重防线。
...
| lint 首轮暴露 manifest、Activity、资源与图标问题 | 质量任务执行过晚，且 warnings-as-errors 包含版本发现噪音 | 源头修复 API/Activity/resource/icon；版本发现由锁定升级流程管理 | lint 在每个 Gate 实现阶段持续执行；依赖版本升级与源码 lint 分离 |
...
| 大量模块和文件尚无 Git 基线 | 先实现后建立可回退点，问题定位只能依赖本地状态 | Gate 1 结束时将组件快照转换为一个干净 Monorepo 基线和 annotated tags | `REPOSITORY_GOVERNANCE.md` 规定组件版本、Monorepo tag、原子协议顺序和回退演练 |
...
| 第三方声明声称无依赖 | 贡献规则没有进入 Gate 必过项 | 登记所有直接 runtime/test/build/CI 依赖及固定版本/许可证/用途 | 依赖变更模板与 CI；notice、lock、verification metadata 同步 |
| 初始组件目录没有托管 CI、不可变公开基线或 Gate 证据 | 实现先于仓库治理，评审结论未绑定公开可验证版本 | 增加根 CI/PR 模板、Gate 证据、SemVer/CHANGELOG 和干净 Monorepo 基线规则 | protected main/required checks 在首次推送后配置；Gate 文档记录 Monorepo tag、组件版本与制品哈希 |
| AVD 只存在本机手工配置 | 分辨率被记录，但 image/tool revision/reset 输入不可重建 | 版本化 AVD matrix、Doctor 和创建脚本 | Gate 证据记录 sdk/emulator revision、wipe/cold boot、设备输出与截图哈希 |
...
Gate 1 的刷新问题同时存在于 UI、模型和消费端，证明只隐藏控件不足以修复语义；没有消费者的枚举、持久字段和 selector 必须一起删除。后续能力必须从真实行为向外暴露，不能从预设配置向内寻找用途。
...
Gate 2 只能在以下条件全部满足后开始：

- Gate 1 代码和架构评审结论为 `approve`；
- 当前评审阻塞项均完成源头修复；
...
- 根仓库建立可追溯的 Gate 1 基线提交和 annotated component/Gate tags；
