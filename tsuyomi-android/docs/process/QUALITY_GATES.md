...
- Select the current interactive resource mode before execution. `LOW` uses `tools/Run-Gradle-Low.bat`, never overlaps Gradle/lint with an AVD or another heavy local check, and remains the default; `HIGH` is explicit and uses `tools/Run-Gradle-High.bat` to consume all logical processors. The two modes schedule identical evidence and never run competing Gradle invocations against one checkout.
