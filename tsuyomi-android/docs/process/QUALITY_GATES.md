...
- Select the current resource mode before execution. `HIGH` explicitly enables local CI and uses all logical processors, Gradle daemon reuse, incremental/build/configuration caches for compilation; device instrumentation remains serial and no competing Gradle invocations target one checkout. `LOW` is the default and permits only hosted CI, not a local preflight or CI-style emulator run. The direct LOW batch runner remains an interactive development tool, not authorization for local CI.
...
- In `HIGH`, local Tier 2 proves the bounded reproduction, then runs the complete planner-selected preflight once before push. Tier 3 hosted CI independently repeats the required plan for protected admission; a local pass never substitutes for a required GitHub check. In `LOW`, skip local CI and use hosted CI only. This mode-specific sequence supersedes both the previous prohibition on local matrix duplication and unconditional local-first execution. A failure returns to its smallest seam, not blind full retries.
