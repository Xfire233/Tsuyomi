...
# Phase 3 plan — local library and migration
...
> **Historical phase boundary after Phase 4 reconciliation:** Phase 3 is complete historical evidence. Where this document conflicts with `DESIGN_DIRECTION_HANDOFF.md` §1.2 or `PHASE_4.md`, the Phase 4 contract is normative. In particular, Phase 4 supersedes Phase 3's post-login import prompt/generic pull wording, split local/source Detail routes and `前往浏览` flow, template/manager navigation, and removal semantics that discarded rating/tags. Active behavior is: login return performs no implicit work; copy/calibration/write are distinct explicit commands; one canonical Detail owns chapters; local removal retains annotations; SystemNode presentation may hide/rebuild/reorder and only Read Later accepts direct membership. The historical paragraphs below remain solely as provenance and do not authorize reverting the active contract.
...
Phase 3 is complete only when a clean API 29 profile can:
...
- Replace the Phase 1 empty-only library root with a real observable library.
...
Phase 4 reconciliation supersedes the presentation requirement above: these stable query definitions become default-created `SystemNode` presentations that may be hidden, rebuilt and manually reordered. Their rules remain immutable; only Read Later accepts direct membership writes. The Phase 4 historical boundary near the document header governs all other conflicting UI/remote/detail/removal instructions below.
...
- Only a verified active source whose signed manifest declares `remoteLibrary.read = true` **and its matching signed request policy** may expose `导入远程收藏`; only one also declaring `writeOperations` containing `add` and its matching policy may expose the optional `新加入书架时同步到网站` setting. The Phase 3 acceptance implementation is Wenku8 `0.2.0`; ESJZone/Yamibo receive neither an inferred capability nor a source implementation in this Phase.
...
- `新加入书架时同步到网站` is off after fresh install, transfer import, Hikari import, login completion and remote pull. It is also off after a source update unless the update has the same trusted publisher and an identical canonical remote capability-set fingerprint; only that preserving update transactionally retains the setting. The user may enable it only after the active package's separate `add` capability grant and a credential-ready source state. Its persistent reference is `(sourceId, trustedPublisherFingerprint, remoteCapabilitySetFingerprint, declared origin)`; package SHA/version remain dispatch provenance rather than policy identity.
...
- Phase 3 never removes, moves or selects remote folders/shelves. Local `移出书架` never mutates the website. No automatic, foreground-open or scheduled remote refresh exists; every later pull is the same explicit `导入远程收藏` action.
...
- Imported subscription configuration is retained as a disabled audit draft only. Phase 3 never creates a remote discovery request; the narrowly scoped remote-favourites actions above do not execute subscription logic.
...
- Smart rules in portable transfer v1; no transfer v2 in this Phase.
...
The baseline already has stable `BookIdentity`, semantic `ReadingProgress`, Room schema 1, manual collection uniqueness/cycle checks, signed source package storage and a Phase 1 empty library screen. Phase 3 must fix these source limitations rather than layer parallel paths:
...
9. Reader portable preferences exist in transfer v1 but have no durable Android consumer; Phase 3 must add the real DataStore state and reader/settings consumption before importing them.
10. Manifest v1 already names `remoteLibrary.read` and `writeOperations`, but Host API/source-contract/runtime expose no typed remote-favourites DTO or capability-enforced read/add invocation. Phase 3 must add one public path; a private Android/Wenku8 shortcut is prohibited.
...
- Manifest v1 extends `remoteLibrary` with signed, canonical per-operation request policies. A policy names the operation, exact HTTPS origin, method, normalized path, referrer and redirect policy, then gives a complete query/form grammar: each declared name appears exactly once and its value is either a canonical fixed literal or a named host binding (`remoteBookId` for add). The sole conditional rule is the declared read `cursor`: it is omitted **iff** the Host API cursor argument is absent, otherwise it appears exactly once with the canonical non-empty opaque cursor the host received from the prior page. This presence rule is canonical policy material and therefore fingerprinted. Extra, duplicate, absent, empty or extension-chosen values are rejected after percent-decoding/canonical re-encoding; no JavaScript value may choose an operation/folder/mode/target literal. A package that requests `read` or `add` without its matching policy is invalid; policy expansion/change is a capability escalation even if the operation name is unchanged.
- Native `HostNetworkGateway.request` receives a host-minted immutable `SourceOperationContext`, not only a generic network grant. It resolves the context’s fixed/host-bound grammar and validates the exact initial request **and every redirect** before the transport sees bytes. A `REMOTE_LIBRARY_ADD` context additionally carries the single-use reconciliation token; every non-add context rejects the add policy surface and its aliases. Extensions cannot mint, copy, widen, select or parameterize an operation context.
- Host API compatibility for the Wenku8 fixture becomes `minInclusive = 1.1.0`, `maxExclusive = 2.0.0`; an older host rejects the package before evaluation. `remoteLibrary.read`, `writeOperations` and operation policies are the authoritative install/update grant declarations.
...
- Add Host API remote-library fixtures for empty/multi-page complete reads, first-page absent versus empty/duplicate/mutated cursor, incomplete page, wrong source/identity, unsafe URL, over-bound page/item count, both add outcomes and malicious add-surface requests from search/detail/chapter/read contexts, redirect aliases, GET/POST variants and altered fixed operation/folder/mode/target literals; every rejected request proves native transport calls equal zero.
...
- Extend conformance to distinguish fatal envelope/graph errors from recoverable independent record warnings and to reject remote-library invocation without the exact active capability grant, policy and operation context.
...
- Add bounded host-owned `RemoteLibraryPage`, `RemoteLibraryItem`, `RemoteLibraryAddRequest`, `RemoteLibraryAddResult`, `SourceOperationContext` and canonical `RemoteOperationRequestPolicy` whose parameters are exact fixed literals or typed host bindings, plus the signed read-cursor omitted-if-absent/once-if-present rule; no raw HTML, arbitrary request body, cookie, remote shelf payload or JavaScript value crosses the boundary.
- `SourceExtensionClient.listRemoteLibrary(cursor)` obtains only a `REMOTE_LIBRARY_READ` context from an active granted read policy; the host, rather than JavaScript, resolves the first-page omission or later exact cursor binding. `addRemoteLibrary(remoteBookId, directActionToken)` obtains only a `REMOTE_LIBRARY_ADD` context from a granted add policy and enabled source policy. The native gateway, not the client method router, validates the exact policy over initial and redirect requests immediately before transport.
- An add token is single-use and bound to source, book identity, reconciliation ID, package digest/version, stable capability-set fingerprint and owner generation. Read/search/detail/chapter contexts never carry it and reject the signed add surface even when their generic network fields otherwise look valid. Context cannot be constructed, copied, widened or selected by extension output.
- The host owns the sole transport-accept callback. It transitions reconciliation from `PENDING_USER_ACTION` to `IN_FLIGHT` before forwarding approved bytes; pre-accept revocation proves `CANCELLED`. Cursor cycles, token replay, mismatched identity, policy violation, source update/revocation, terminal QuickJS failure, deadline, cancellation and close follow existing per-operation isolation. No runtime/executor survives its feature owner.
...
- `ExtensionInstaller` retains its package-specific approval receipt for activation, but separately computes the full stable remote capability-set fingerprint and diffs signed operation policies. Identical same-publisher policies preserve user writeback configuration; policy expansion/change, origin change, capability removal/addition, key change, revocation and downgrade surface approval/disable semantics rather than being hidden by a package-SHA or delta-only fingerprint.
...
source_remote_policy(sourceId, trustedPublisherFingerprint, remoteCapabilitySetFingerprint,
...
remote_library_reconciliation(id, sourceId, remoteBookId, operation=add,
                              packageDigest, packageVersion, capabilitySetFingerprint,
                              registryGeneration, state, createdAt, updatedAt,
                              safeCode?, diagnosticId?)
...
- System collections are fixed query definitions, never mutable membership rows; Phase 4 may hide/rebuild/reorder their presentation nodes, and only Read Later gains explicit membership.
- Collection parent graph is acyclic and depth-bounded in Phase 3; Phase 4 tightens presentation depth to two levels. Order is repaired transactionally after move/delete.
- Smart collections reject direct membership writes.
- Subscription rows remain `enabled=false` and have no execution method in Phase 3.
...
- Confirmed Transfer/Hikari import uses the durable cross-store journal defined below. Room mutations remain one off-main transaction, but Phase 3 does not claim that Room and DataStore share a physical transaction.
- Import warning/audit records are redacted and contain no credential values or raw invalid payload fragments.
- `source_remote_policy` contains no credential or cookie data. It stores the stable canonical `remoteCapabilitySetFingerprint` = source ID + trusted publisher fingerprint/key ID + complete signed remote read/write operation policies + applicable declared origins; it excludes package SHA/version/delta. The installer retains a package-specific approval receipt separately. A same-publisher, policy-identical update transactionally rebinds and retains enabled add; fresh install, policy expansion/change, add/read removal, origin change, unauthorized key change, revocation and local downgrade disable it and require new approval.
- `remote_library_reconciliation` is created only by the direct source-detail `加入书架` handler in the same Room transaction as local membership. At most one nonterminal add exists per book/source. `PENDING_USER_ACTION` becomes `CANCELLED` only when the token is revoked before native transport acceptance and the host proves no request bytes were offered. Native acceptance is the sole linearization point: it compare-and-sets to `IN_FLIGHT` before handing the approved request to transport. Every post-accept outcome except parsed typed `applied` / `already-present`—including timeout, cancellation, owner/process death, package/grant change, malformed response, typed source rejection or parse failure—is `UNRESOLVED`. Retry always mints a new token.
...
Room and DataStore cannot share one physical transaction. Phase 3 therefore guarantees crash-atomic **user exposure** with an application-owned recovery gate and an idempotent journal.
...
- Add an application-owned `InstalledSourceRegistry` over verified `InstalledExtensionStore` records. It issues immutable leases `(sourceId, packageDigest, packageVersion, remoteCapabilitySetFingerprint, registryGeneration)`.
- Install, update, remove and revocation take one per-source lifecycle mutex, advance the generation and transactionally synchronize `source_availability`. A remote pull captures a lease but holds no lifecycle lock during network aggregation; its final complete Room merge takes the same mutex, rechecks that lease at Room begin and immediately before commit, and applies all rows once or discards all rows. Lifecycle change and pull are therefore linearly ordered without a long network lock.
- Phase 3 adds no independent source-enabled toggle. A source is available only when a verified active package is present; otherwise its books are dormant.
- Remote-policy reconciliation runs with the same verified snapshot. Missing/revoked package, publisher/grant/policy mismatch, removed `read`/`add`, origin change or generation change disables the affected action before UI dispatch; it does not delete local books or progress.
- Imported ESJZone/Yamibo books remain dormant because Phase 3 does not install those sources. No library query opens an extension or performs network I/O.
...
- Show signed capability/grant status, credential-ready state and one explicit `导入远程收藏` action. Show the default-off `新加入书架时同步到网站` switch when `add` is currently granted; if read remains valid but a previously enabled add policy becomes invalid, retain it disabled with `来源更新后需要重新授权`. Enabling requires a confirmation naming the source, website effect and local-only removal behavior.
...
- Reconciliation state appears on source and local book details as `等待确认同步` (`PENDING_USER_ACTION` / `IN_FLIGHT`), `已同步` (`CONFIRMED`), `网站未确认` (`UNRESOLVED`) or `未发送到网站` (`CANCELLED`). Only unresolved and cancelled states offer `重试同步`, and only while a current add capability remains valid. No color-only status is used.
...
- Bump the deterministic signed Wenku8 fixture extension to `0.2.0`, Host API minimum `1.1.0`, declare `remoteLibrary.read = true`, `writeOperations = ["add"]`, and provide signed exact read/add request policies. The install/update approval UI must display capability/policy additions, retain the package-specific receipt and derive stable runtime grant identity from the canonical effective policy set.
...
- Phase 3 import is merge-only; it never deletes host records merely because they are absent from an import.
...
- A complete remote pull is merge-only. It captures one registry lease, aggregates under fixed bounds without holding the lifecycle mutex, then takes that source’s mutex for a begin-and-precommit lease check plus one Room transaction. If lifecycle change linearizes first, every pulled row is discarded; if the pull linearizes first, its complete result commits once before lifecycle update. It never deletes local state, overwrites local rating/tags/progress/manual organization, enables writeback or modifies remote data.
- A direct local add with enabled writeback atomically creates local membership plus `PENDING_USER_ACTION`. The native transport-accept point changes it to `IN_FLIGHT`; only pre-accept cancellation/revocation with proof of no offered bytes produces `CANCELLED`. `CONFIRMED` requires the semantic result. Every other post-accept result—including typed source rejection, timeout, cancellation, owner/process death, package/grant change, malformed response or parse failure—is `UNRESOLVED`. No state is labelled synchronized before `CONFIRMED`.
- State/UI/action contract: `PENDING_USER_ACTION` / `IN_FLIGHT` → `等待确认同步`, no concurrent retry; `CONFIRMED` → `已同步`, no retry; `UNRESOLVED` → `网站未确认`, explicit new-token `重试同步`; `CANCELLED` → `未发送到网站`, explicit new-token retry. Retry controls disable with the existing reapproval explanation when add is no longer valid. Every label is persistent text, TalkBack-announced and survives recreation.
...
| 手动集合编辑 | Phase 3: visible for manual collections only. Phase 4 amendment: system query definitions still cannot be renamed/deleted, but their presentation nodes may hide/rebuild/reorder; only Read Later accepts direct membership. Smart collections reject direct membership writes. |
...
| 来源订阅刷新 | Hidden in Phase 3 because no discovery consumer exists. Imported drafts are visible read-only only to explain retained blocked data. |
...
- Source remote library: capability/grant absent, credential absent, first-login import question, manual pull confirmation/loading/empty/success, cancellation, session/verification required, incomplete/limit/source-change failure, default-off add setting confirmation, `PENDING_USER_ACTION` / `IN_FLIGHT` waiting, confirmed, unresolved and cancelled add states, plus their explicitly enabled/disabled retry actions.
...
- Remote favourites remain host-mediated. Credential readiness is an opaque host result; extensions never see Cookies, WebView state, account names or raw pages. The signed full operation policy, immutable `SourceOperationContext` and exact remote capability are separately approved/revalidated; native transport validates every initial/redirect hop before bytes cross the process boundary.
- No automatic remote call exists: initial prompt, pull, add and retry each require a direct visible user action. The transport-accept callback is the cancellation linearization point; only proven pre-accept revocation is cancelled, while every post-accept uncertainty is unresolved. A late owner/source/package/grant callback cannot target a later click or create a retry.

## Implementation sequence and revert boundaries

Each commit includes its tests and affected documentation.

1. `test(protocol): complete Phase 3 transfer, Hikari and remote-library fixtures`
...
3. `feat(database): migrate local library schema for Phase 3`
...
9. `test(phase3): record end-to-end admission evidence`
   - Phase document evidence, AVD recipes, checksums and known boundaries.

Rollback is forward-only whenever an import journal or remote reconciliation may exist. A rollback build hides Phase 3 import/export and remote-library entry points/settings, retains `import_session`, normalized-plan parser/digest verification, `lastAppliedImportDigest`, pre-navigation coordinator, recovery/abort UI and import/export sweepers. It also retains `source_remote_policy` and reconciliation rows but dispatches **no** remote call; any `PENDING_USER_ACTION` / `IN_FLIGHT` row becomes visibly `UNRESOLVED`, while every remote reconciliation label and retry control is hidden with the disabled remote-library UI. Every build accepting a direct upgrade from Phase 3/schema 2 retains these compatibility paths; it never relies on release-wide state assertions or telemetry. A future on-device database-open/pre-navigation migration may replace them only after consuming every pre-existing journal/reconciliation state and referenced file form before `NavHost`.
...
- Host API 1.1 remote-library contract fixtures: bounded complete list pagination, first-page absent cursor versus empty/duplicate/mutated cursor rejection, malformed/incomplete/looped cursor rejection, unsafe URL/source/identity rejection, add identity/outcome validation, stable full-policy fingerprints and update diff behavior. Malicious fixtures alter fixed operation/folder/mode/target values, bindings, names, duplicate values, redirect aliases and GET/POST from search/detail/chapter/read; native transport calls must stay zero absent the exact add context/token.

### Room / Android unit and instrumentation

- Schema 1→2 migration preserves every Phase 2 book and semantic progress row; nested collection rows/order remain exact, schema-1 manual members receive deterministic `library_entry`/membership backfill, and nonmember progress rows remain non-library.
...
- Remote policy/reconciliation tests prove default off after fresh/transfer/Hikari/install/login/pull; same-publisher policy-identical update retains the switch; policy expansion/change, read/add removal, origin change, unauthorized key change, revocation and downgrade disable it with zero dispatch. They prove at-most-one nonterminal add; local membership atomic with operation; direct-token single use/source/book/reconciliation/version/generation binding; no automatic retry; local removal has no remote call; retry produces a new action/token.
- Barrier tests inject lifecycle change after the final response, parse, Room begin and immediately before commit: either a complete old-lease merge linearizes first or zero pulled rows apply. Transport barriers cover token mint, pre/post acceptance, headers/body send, server apply, parse and owner/process cancellation: only pre-accept becomes `CANCELLED`; every post-accept uncertainty is `UNRESOLVED`. Every state has process-recreation, TalkBack/action-availability and golden coverage.
...
- Direct-upgrade matrix opens every Phase 3 import-journal and remote-reconciliation state/file snapshot in the current schema-2-compatible build, skipping any feature-disabled intermediate. It recovers or safely marks unresolved before `NavHost`, without a remote call or retained journal. A direct upgrade never reuses a package-specific approval receipt as a stable source policy grant.
...
1. Install signed Wenku8 `0.2.0` fixture and approve its displayed `remoteLibrary.read` / `add` capability and request-policy expansion. Verify a source without the matching grant/policy has neither pull nor writeback control.
...
> Phase 3 历史证据边界：最终 fixture 用户流留下了 E-ink 横屏截图，但没有形成标准手机 `1080×2400` 竖屏与 E-ink `1264×1680` 竖屏两条独立验收记录，因此不得把既有横屏截图表述为这两条 portrait 证据。自此规则修订后的后续 Phase/PR 必须执行 [`AVD_MATRIX.md`](../verification/AVD_MATRIX.md) 的双竖屏基线；横屏只能作为附加窗口验证。
...
- Remote library covers missing read/add/policy/context/token, disabled writeback, absent credential, session/verification required, user declines first import, cancel before pull apply, empty/multi-page success, cursor loop/incomplete/5,000-record or 8 MiB aggregate bound, source/identity mismatch, update/revocation races at final apply, malicious altered-literal/add-surface/redirect/GET/POST attempts from non-add operations, stable-policy update versus policy change, token replay, pre/post transport acceptance, typed rejection, ambiguous timeout/cancellation, explicit retry, local removal and process death. Every forbidden/failed path proves zero unintended remote request and no false synchronization claim.
...
- API 29 hosted instrumentation actually installs/starts the emulator and runs Phase 3 product tests.
...
| Source causes an implicit or unauthorized remote write | Signed full per-operation policy with complete fixed-literal/host-binding parameter grammar and separately approved capability; immutable host-minted context; native initial/redirect validation before transport; default-off policy; single-use direct-action token; malicious altered-value/read/search/detail/chapter/redirect/GET/POST tests assert zero calls. |
| Local and remote outcome diverge or a timeout duplicates an add | Atomic local membership/reconciliation row; host-owned transport-accept linearization; typed semantic `applied`/`already-present` result; only proven pre-accept cancellation is cancelled, all post-accept uncertainty is unresolved; retry only by new direct action; no false synchronization label. |
| Extension update/revocation races remote actions or changes write semantics | Per-source lifecycle mutex and lease recheck at pull Room begin/precommit; package-specific approval separate from canonical capability-set fingerprint; identical policy update preserves explicit setting, every capability/policy/origin/key/downgrade change disables dispatch and requires approval; late/in-flight add becomes unresolved; no automatic retry. |
...
| Room migration blocks existing Phase 2 users | Exported schema-1 fixture; deterministic membership backfill before new FK/query invariants; exact hierarchy/order/book/progress assertions; explicit nonmember-progress behavior; no destructive fallback. |
| Large import/export or remote pull causes allocation/partial output | Existing 32 MiB input/export bounds; remote read page/count/8 MiB aggregate bounds; complete-then-lease-linearized transaction apply; capped preflight/sweeper; no partial or stale-package remote apply. |
...
| D5 | **[HISTORICAL — superseded by Phase 4 D19]** Phase 3 removed local library membership/rating/tags while preserving progress/history. Phase 4 instead retains rating/local tags and removes only the explicitly named presence origins. | Local removal never removes a remote favourite. |
