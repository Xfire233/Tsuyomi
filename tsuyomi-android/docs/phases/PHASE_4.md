...
- Current implementation target: `org.tsuyomi.android` `0.2.0` / versionCode `2`; Host API `1.1.0`; HXP manifest v1 with signed `update-check-v2`; deterministic Wenku8 fixture `0.2.30`; Room schema `10`; current transfer output `tsuyomi-transfer` v3 with strict v1/v2/v3 import. This does not replace the canonical APK, database or installed extension.
...
The user authorized the combined Library filter/sort panel, restrained tap-only fixed-Tab transitions, and activating Detail's selected `已在书架` primary to request the existing local-removal confirmation. After the v9 delete path was found to lose rating, tags and Read Later, the user explicitly chose to repair storage and migration before exposing that action. This packet includes additive Room9→10 local-pin retention, independent Read Later projection, application membership guards and lossless transfer-v3 pin representation with legacy v1/v2 input. The owning product rules are Constitution G-15/G-25/G-31/G-33, L042-M15/M18/M19 and B042-B4; storage ownership is in SHELVES.md and the versioned transfer schema.
