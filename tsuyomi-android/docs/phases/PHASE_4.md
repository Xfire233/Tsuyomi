...
Library remains the first corrected production surface. Its AppBar, shortcut shelf, three-column cover grid, menus, selection grammar and dedicated child routes are driven by production Room, collection, read-later, refresh and navigation owners. `LibraryProductionJourneyInstrumentedTest` remains the highest observable seam and rejects restoration of the interim mixed-node-card screen.
...
2. One explicitly enabled website-mirror root per source, showing that source account’s remote folder/content snapshot. The mirror root and remote folders are independent pinnable Library shortcuts and use the shared Library folder/book visual system. Manual calibration and typed confirmed foreground writes update the local projection; no background pull occurs.
