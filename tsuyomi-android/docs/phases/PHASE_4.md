...
- Independent automation `c5ad2ee3095e74af04c0a3f8beaec5befda40458` is public and passed hosted contributor CI. Main now requires CI and code-owner review; `official-distribution` deploys only from main. Distinct production signing secrets were provisioned with verified Windows-account-bound encrypted local backups; portable/off-machine recovery is not claimed.
- [First publication run](https://github.com/Chachaanteng/tsuyomi-extensions/actions/runs/34562888521) failed at the corresponding-source archive request with HTTP415, before formal package/catalog publication. [Repair PR1](https://github.com/Chachaanteng/tsuyomi-extensions/pull/1) corrects API media negotiation; its HTTP regression fails before the fix and passes afterward, with48/48 local Node tests. Publication is paused pending legitimate code-owner review, not bypassed to self-approve a sole-maintainer PR.
