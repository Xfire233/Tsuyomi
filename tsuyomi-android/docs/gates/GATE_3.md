...
- Manifest v1 extends `remoteLibrary` with signed, canonical per-operation request policies. A policy names the operation, exact HTTPS origin, method, normalized path, allowed/required query or form names, which values are bound to the host `remoteBookId`/cursor, referrer policy and redirect policy. A package that requests `read` or `add` without its matching policy is invalid; policy expansion/change is a capability escalation even if the operation name is unchanged.
- Native `HostNetworkGateway.request` receives a host-minted immutable `SourceOperationContext`, not only a generic network grant. It validates the initial request **and every redirect** against the context’s signed request policy before the transport sees bytes. A `REMOTE_LIBRARY_ADD` context additionally carries the single-use reconciliation token; every non-add context rejects the add policy surface and its aliases. Extensions cannot mint, copy, widen or select an operation context.
...
- Add bounded host-owned `RemoteLibraryPage`, `RemoteLibraryItem`, `RemoteLibraryAddRequest`, `RemoteLibraryAddResult`, `SourceOperationContext` and canonical `RemoteOperationRequestPolicy`; no raw HTML, arbitrary request body, cookie, remote shelf payload or JavaScript value crosses the boundary.
...
- Reconciliation state appears on source and local book details as `等待确认同步` (`PENDING_USER_ACTION` / `IN_FLIGHT`), `已同步` (`CONFIRMED`), `网站未确认` (`UNRESOLVED`), `同步失败` (`FAILED_SAFE`) or `未发送到网站` (`CANCELLED`). Only unresolved, safe-failed and cancelled states offer `重试同步`, and only while a current add capability remains valid. No color-only status is used.
...
- A direct local add with enabled writeback atomically creates local membership plus `PENDING_USER_ACTION`. The native transport-accept point changes it to `IN_FLIGHT`; only pre-accept cancellation/revocation with proof of no offered bytes produces `CANCELLED`. `CONFIRMED` requires the semantic result. Post-accept timeout, cancellation, owner/process death, package/grant change, malformed response or parse failure is `UNRESOLVED`; parsed typed proof of no application is `FAILED_SAFE`. No state is labelled synchronized before `CONFIRMED`.
- State/UI/action contract: `PENDING_USER_ACTION` / `IN_FLIGHT` → `等待确认同步`, no concurrent retry; `CONFIRMED` → `已同步`, no retry; `UNRESOLVED` → `网站未确认`, explicit new-token `重试同步`; `FAILED_SAFE` → `同步失败`, explicit new-token retry; `CANCELLED` → `未发送到网站`, explicit new-token retry. Retry controls disable with the existing reapproval explanation when add is no longer valid. Every label is persistent text, TalkBack-announced and survives recreation.
...
- Source remote library: capability/grant absent, credential absent, first-login import question, manual pull confirmation/loading/empty/success, cancellation, session/verification required, incomplete/limit/source-change failure, default-off add setting confirmation, `PENDING_USER_ACTION` / `IN_FLIGHT` waiting, confirmed, unresolved, safe-failed and cancelled add states, plus their explicitly enabled/disabled retry actions.
...
- Host API 1.1 remote-library contract fixtures: bounded complete list pagination, malformed/incomplete/looped cursor rejection, unsafe URL/source/identity rejection, add identity/outcome validation, stable full-policy fingerprints and update diff behavior. Malicious fixtures attempt the signed add surface and redirect aliases from search/detail/chapter/read plus GET/POST variants; native transport calls must stay zero absent the exact add context/token.
...
- Remote library covers missing read/add/policy/context/token, disabled writeback, absent credential, session/verification required, user declines first import, cancel before pull apply, empty/multi-page success, cursor loop/incomplete/5,000-record or 8 MiB aggregate bound, source/identity mismatch, update/revocation races at final apply, malicious add-surface/redirect/GET/POST attempts from non-add operations, stable-policy update versus policy change, token replay, pre/post transport acceptance, add-safe failure, ambiguous timeout/cancellation, explicit retry, local removal and process death. Every forbidden/failed path proves zero unintended remote request and no false synchronization claim.
...
| Source causes an implicit or unauthorized remote write | Signed full per-operation policy and separately approved capability; immutable host-minted context; native initial/redirect validation before transport; default-off policy; single-use direct-action token; malicious read/search/detail/chapter and redirect/GET/POST tests assert zero calls. |
...
- The following remediation is awaiting renewed independent review: signed per-operation policy plus host-minted native `SourceOperationContext`; per-source lifecycle lease/mutex merge; canonical stable capability-set fingerprint separate from package approval; exhaustive transport-accept state/UI contract and barriers.
