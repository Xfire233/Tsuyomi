...
- Add bounded host-owned `RemoteLibraryPage`, `RemoteLibraryItem`, `RemoteLibraryAddRequest`, `RemoteLibraryAddResult`, `SourceOperationContext` and canonical `RemoteOperationRequestPolicy` whose parameters are exact fixed literals or typed host bindings; no raw HTML, arbitrary request body, cookie, remote shelf payload or JavaScript value crosses the boundary.
- `SourceExtensionClient.listRemoteLibrary(cursor)` obtains only a `REMOTE_LIBRARY_READ` context from an active granted read policy. `addRemoteLibrary(remoteBookId, directActionToken)` obtains only a `REMOTE_LIBRARY_ADD` context from a granted add policy and enabled source policy. The native gateway, not the client method router, validates the exact policy over initial and redirect requests immediately before transport.
