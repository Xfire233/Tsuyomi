...
10. **Temporary profile execution policy:** `.agents/skills/tsuyomi-android-review/review-policy.json` alone selects profiles executed by routine review. Active profiles receive the current evidence matrix; every deferred profile retains its artifacts, assertions, fixtures and node obligations without new design, golden, approval or readiness claims until the policy's explicit restoration pass.
...
| 8 | `library/mirror/{bindingId}` + optional remote-folder child | CL | L C E Er Off Ref Sel Mut Unr Mod Tut Fs2 | Same shared Library book renderers and grid/list/compact interaction system. Default simple mode is one aggregate `全部网站收藏` with no folder nodes, folder pins, target picker or MOVE controls. Per-source overflow opt-in enables folder-first root content, dedicated folder pages, independent folder pins and target-aware operations. AppBar owns refresh, root pin, layout, applicable sort and grouping toggle. No nested root shortcut shelf. |
...
| 13 | Browse source-home website-shelf deep link | CL | L C E Er Off Ref Sel Mut Unr Mod | Opens the same canonical website-mirror surface under its `bindingId`; defaults to the aggregate simple presentation unless that source explicitly enabled grouping or retains prior folder-pin intent. No second simplified remote-library list or tabs presentation. |
...
| B | Detail + chapters | Prove the 135×180dp cover and Title-role title limited to two default lines. Only the unmodified book title participates in overflow measurement: a title that ends on line two remains complete and shows no affordance; a genuine third-line overflow reserves one trailing 48dp target with a text-scale circular Material Info icon, expands the full title in place and collapses it again. Status is plain `bodyMedium` metadata at the start of the same compact row as the ununderlined muted-blue author link and optional source update date; status/date share colour and metrics, and no Badge/chip/container remains. A true two-line title plus one-line metadata keeps rating and SplitButton inside cover height with exactly 8dp metadata→actions and rating→SplitButton gaps; controls never bottom-align to manufacture internal whitespace. Older extensions receive no invented date. Rating remains 36dp/20dp/28dp with native touch expansion. Non-stacked controls stay intrinsic; only two-row fallback fills width. |
...
| F | Website mirror Library surface | Default simple mode uses the same Library grid/list/compact cards, spacing, cover loading, selection, scroll FAB and drag grammar as local Library while showing one aggregate `全部网站收藏`; no folder chrome or MOVE target is visible. A cover first learned by canonical Detail upgrades the shared mirror catalog entry, appears immediately after Back, and survives later remote-list refreshes that still omit a cover. The mirror overflow enables grouping per source without changing website data. Grouped mode restores folder-first root content, dedicated pages, independent root/folder pins, root target picker, exact-folder ADD→MOVE and intra-mirror MOVE. Both modes retain local copy, ADD-to-default, confirmed website removal and immediate pin/write feedback. |
...
| F5 Mirror partitions | Website mirror with aggregate simple-mode projection, per-source grouping opt-in, remote folder membership retained while hidden, independent root/folder shortcut pins, folder-pin migration intent, frozen missing target, manual refresh, confirmed local projection updates, compound ADD→MOVE partial state and one unrelated accepted unresolved write; local partition absent/present only after explicit creation | route #8; S8 |
...
| F8 Sources/remote | Installed and gated sources; 100-book remote mirror with per-book target membership; simple-mode aggregate, explicit grouping enablement, target loading, local copy selection and exact single-book website operations | routes #9/#11–14; boards B/F; S5/S12/S17 |
...
| F12 Preferences/tutorials | Unknown schema + route-owned introductions + per-source website-grouping preference with default-off, folder-pin migration and disable/re-enable restoration fixtures; concise and task-specific | routes #4/#6/#8/#9/#18/#20; S8/S11/S17 |
...
| S5 † | Detail → source date present/missing → author link → applied same-source author results → result Detail → Back with no repeat search → one-line and two-line cover-height headers → three-line title relocates both controls → width/font pressure exercises horizontal and stacked fallback arrangements → rating/add/dropdown/read-later → expand/collapse introduction → volume/unread/sort → exact chapter Reader entry | Source date is not a local progress/cache timestamp; date omission leaves no placeholder; native link has accessible activation. Full title remains readable. Normal one-/two-line states use the cover-right space; only measured third-line/metadata/width pressure relocates the controls. Rating/SplitButton keep 48dp visual containers, theme-large corners and intact actions; relocated upper/lower gaps both equal 8dp. Stars retain native 48dp touch expansion, and the menu stays anchored after relocation. |
...
| S8 † | Library root → enter a new source mirror in default simple mode → verify one aggregate `全部网站收藏`, no folder/picker/MOVE chrome and direct ADD-to-default → enable website grouping from mirror overflow → verify folder-first root, folder pages, independent pins, root target picker, exact-folder ADD→MOVE and intra-mirror MOVE → disable/re-enable and verify hidden folder pins/order/membership restore → recreate process with and without prior folder pins → exercise local copy, confirmed REMOVE, multi-book/cross-source rejection and ADD-confirmed/MOVE-unresolved recovery | Default mode removes unused grouping complexity without flattening website data. Only persisted folder-pin intent auto-retains grouped migration state. Both modes retain shared Library rendering, root pin, refresh and immediate feedback; grouped mode alone exposes folder/MOVE workflows. |
...
| S10 † | Enter query → verify inert → trailing Search submit → local/remote unified results → source failure; separately activate Detail author link → automatically applied author results → Detail/Back | Ordinary draft editing/restoration produces no query. Author activation is one explicit source-bound author submission, not title search; Loading/results appear without a second confirmation. Return/restore does not replay it, and manual verification/retry preserves the applied author query/type. Existing exact-identity merge and source-scope obligations remain. |
| S11 † | Rule editor all control types + invalid AST | Correct M3 controls, caps, applicability and draft restoration |
| S12 † | Browse Source Home → website shelf deep link → canonical simple mirror root → explicit refresh → select and locally copy a batch → open a remote book Detail → use the real M3 SplitButton primary local-root action → open its anchored dropdown → toggle read-later or one local destination → choose the single aggregate website destination and ADD-to-default → enable grouping for that source and reopen the menu → choose one exact website target | Browse and Library expose one canonical mirror presentation. Detail primary changes to disabled `已在书架`; trailing segment remains operable. Secondary placement actions execute from the same compact labelled dropdown and no destination bottom sheet exists. Simple mode avoids target-choice complexity; grouped mode restores single-target ADD/ADD→MOVE. No automatic route-entry/background read or website-data flattening. |
...
Board B / Scene S5 distribution addendum: `B042-B14` owns the six-band cover-right distribution; `B042-B15` removes the rating frame; `B042-B16` aligns the first visible star edge—not its invisible band—to the title, author, source metadata and SplitButton left edge. At the Standard phone baseline, fill the 135×180dp cover-right column in this order: one/two title lines; author; status plus optional source date; borderless stars; framed 48dp SplitButton ending at the cover bottom. Keep 20dp star glyphs and 28dp centres. Expanded touch bounds may extend invisibly into the existing gutter but may not overlap the cover. Expanded title, wrapped metadata, large font or insufficient width still relocates by measurement.
Board B / Scene S5 author-link addendum: `B042-B12` removes the persistent underline and replaces saturated browser blue with the shared muted-blue link token (`#4A6E8A` light / `#A8C4D8` dark). The exact author text, native link semantics, same-source search, keyboard/TalkBack activation, visible failure and no-lifecycle-replay behavior remain unchanged. Do not substitute a chip, icon, primary teal or bold weight.

The available-width proof includes actual system-bar and cutout insets (§12 of the Constitution): in landscape, verify the entire borderless rating row, SplitButton disclosure and reading FAB remain inside the safe horizontal viewport. AppScaffold consumes horizontal safe-drawing insets once; M3 chrome must not add them again.

At fontScale 2.0, plain publication status and source date must remain complete and unclipped; when the six-band cover-right composition no longer fits, it uses measured fallback rather than shrinking typography or restoring a Badge.
