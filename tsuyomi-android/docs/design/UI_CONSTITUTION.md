...
| F · Remote Library | Website-mirror root and remote-folder children use the same shared Library folder/book cards, grid/list/compact renderers, selection, drag targets, sort/layout actions and scroll affordance as the local Library. Mirror folders are nodes-first on the root, followed by aggregate remote books; each folder opens a dedicated page. Root/folder pins are independent shortcut items. Local→remote drag is single-book; remote→local is copy; remote MOVE and REMOVE remain visibly distinct. | Simplified checkbox/card list; remote-folder tabs; a parallel remote visual language; a recursively repeated root shortcut shelf; batch website writes; background target/list reads. |
...
| ID | Decision |
|---|---|
| `L042-M1` | A website-mirror root is always a first-class Library shortcut target with stable typed identity. Returned remote folders become first-class independently pinnable targets only while that source's optional website-grouping mode is enabled. Pinning one never implicitly pins another. New pins follow existing shortcuts from the same source, otherwise append at the shelf end. Turning grouping off hides but does not delete folder pins or their order; if a pinned remote target disappears, it remains frozen and actionable when grouping is enabled. |
| `L042-M2` | **Supersedes the always-visible remote-folder presentation.** Every source defaults to a simple mirror route titled `网站收藏`; its single website destination is labelled `全部网站收藏`, and the shared Library book grid/list/compact renderers aggregate every remote book regardless of website target membership. Simple mode shows no remote-folder nodes, folder pages, folder pins, target picker or MOVE controls. The mirror overflow offers the per-source opt-in `启用网站分组`. Grouped mode restores folder-first root content, dedicated folder pages, independent folder pins, target-aware operations and the same shared Library visual/interaction system; neither mode renders a second simplified list, tabs or the Library-root shortcut shelf inside the mirror page. |
| `L042-M3` | In simple mode, a single same-source local book dropped on the mirror-root shortcut directly requests ADD to the website's actual default target; no target sheet is shown. In grouped mode, the same root drop opens the bounded target sheet with the default website shelf preselected, while a remote-folder shortcut addresses that exact target. A multi-book payload cannot enter any website target and receives `网站操作仅支持单本`; cross-source books are rejected before authorization or transport. Every drag action has an equivalent labelled button/menu action. |
...
| `L042-M7` | Remote target/list reads never run in the background or merely because a route is restored. Opening Detail's split-button dropdown is itself the explicit foreground read action and may load current website targets; mirror pages refresh only from their visible refresh action or an accepted foreground write result. Typed confirmed ADD/MOVE/REMOVE results immediately update the local mirror projection and shortcut counts; the next explicit refresh calibrates them. Simple mode retains target IDs in the local snapshot without presenting folder structure, so enabling grouping never requires destructive website migration. |
| `B042-B1` | **Its destination-sheet surface is superseded by `B042-B4`; the action semantics remain binding.** Canonical Detail uses the real Material 3 `SplitButton`. The primary segment performs only the immediate local-root add; after local presence it displays disabled `已在书架`. Local shortcut/manual-collection destinations, website destinations and read-later remain explicit independent operations; local and website writes are never combined into one opaque transaction. |
...
| `B042-B3` | **Direct mirror-action correction.** A website-mirror root never uses an icon-only `+` to mean `固定到快捷书架`, nor an icon-only trash can to mean `移出快捷书架`; those symbols are too easily misread as adding/removing books. Mirror shortcut management is a labelled Material overflow action with the standard pin icon. `全部复制到本地书架` / `复制所选到本地书架` uses the distinct copy icon; grouped-only `移至网站分类` uses move-to-folder; destructive `从网站收藏移除` uses delete plus the complete error-coloured verb and still requires confirmation. Selection mode replaces refresh/layout chrome with select-all, local-copy and applicable single-book website actions. Direct non-selection AppBar icons remain reserved for universally recognizable refresh and layout controls. |
| `B042-B4` | **Direct canonical-device correction superseding the separate read-later action and the sectioned destination sheet; `B042-B6` supersedes the former fixed placement/alignment and `B042-B8` supersedes the former 56dp FAB-matched Detail height.** Detail uses exactly one official Material 3 `SplitButton`, with theme-large outer corners (currently 16dp), 24dp icons and `labelLarge` text. Leading icon/text spacing and content padding follow the extended FAB; the split's 4dp inner seam remains distinct. Outer corners derive from the same theme shape as the FAB, not a separate fixed radius. Geometry consistency does not duplicate FAB elevation or change action colours, disabled feedback or emphasis: reading remains the floating primary action. The primary segment is the immediate local-root `加入书架 / 已在书架` action. The trailing standard disclosure opens one compact M3 `DropdownMenu` anchored below the control; it never opens a bottom sheet or animates upward from the screen edge. The menu provides labelled immediate actions for `稍后再读`, shortcut/manual-collection destinations, available website destinations, partial-move recovery and future secondary placement actions. Read-later has no standalone body button; it stays local-only and atomically creates local presence for an unpinned book. Rows expose complete labels and checked/selected state, close after invocation, and preserve the trailing segment after the primary becomes disabled. |
...
| `B042-B9` | **2026-09-05 intrinsic-width correction; `B042-B14` supersedes baseline placement but retains its width rule.** Rating and SplitButton use complete measured intrinsic widths and align to the metadata/content start edge; neither consumes leftover row width. Split width follows the full localized label, 24dp leading icon, established padding, 4dp seam and fixed 48dp disclosure. Only the exceptional relocated fallback that becomes two vertical rows fills available width. Localization, local-presence text and title expansion remeasure without clipping. |
...
| `L042-M8` | Website grouping is a local, per-source presentation preference owned by `core:preferences`, default off for sources without explicit prior folder-pin intent, never transferred and never written to the website. During migration, a source with at least one persisted remote-folder shortcut pin starts in grouped mode; merely having remote target metadata or books assigned to website groups does not opt in. Explicitly disabling grouping hides and preserves folder pins, ordering and target membership so re-enabling restores them. The root shortcut, aggregate mirror, refresh, local copy, ADD-to-default and confirmed REMOVE remain available in both modes. |
| `L042-M9` | **2026-09-06 shortcut expansion and drag-preview correction.** The resting inline shortcut shelf remains the established 80×116dp tile. Opening `全部快捷书架` uses the same regular-phone three-column, 3:4 cover-led card geometry and 8dp grid gaps as the root Library book grid, with a restrained top-anchored expansion/fade and instant reduced-motion replacement. Expanded cards do not reuse the compressed inline 76dp media field. Shortcut and shortcut-book drag ghosts are compact horizontal 216×64dp summaries with a small 3:4 thumbnail/icon and label, never a full cover-sized card that obscures drop targets. |
...
Website mirrors use the same dedicated-page folder model and shared visual/interaction system as local Library contexts. The mirror root renders remote folder nodes first and then all remote books; each folder and book uses the shared grid/list/compact renderer, selection grammar, drag preview, target feedback, sort/layout persistence and direction-aware scroll FAB. Opening a remote folder pushes its own page instead of expanding a tree or switching a tab. Mirror pages own a source/folder AppBar with refresh, current pin state and Library layout actions, but never recursively render the Library-root shortcut shelf. The `本地整理` node appears only after explicit creation, opens as a separate local page, never masquerades as website structure and never causes website writes.
...
Current production Search exposes only the bounded query, trailing submit action, layout/sort and one aggregate progress indicator. D33 public/local/source-specific filter descriptors, advanced filter UI and `search-v2` capability negotiation are deferred and impose no current implementation dependency.

The explicit Detail author-link action (`B042-B5`) is also a submission: it creates one source-bound author search and applies it immediately. This exception authorizes only the user's activation, not automatic querying on route entry, draft edits, Back, recomposition or process restoration. The applied author query/type survives the manual-verification continuation; ordinary trailing Search remains the normal draft submission.
...
| `library/mirror/{bindingId}` and remote-folder child | Library | CL | Source/folder name + count/status / Up→parent mirror page or Library / refresh, pin, layout; sort overflow where books exist | Same shared Library folder/book cards and grid/list/compact renderers. Mirror root is remote folders first then aggregate books; each folder opens a dedicated page. No nested root shortcut shelf. Root/folder pin independently into the root shortcut shelf. Single-book drag supports mirror-root target picker, exact folder ADD→MOVE, intra-mirror MOVE, local copy and confirmed website REMOVE; covers carry unresolved/frozen state. | Scroll / explicit pages | No route-entry/background read; opening Detail destinations is explicit foreground target loading. Confirmed writes update the local projection immediately. Missing pinned targets freeze instead of disappearing. |
...
| `source/verification` | Browse | FL | `登录验证` / Up / – | Host notice + controlled WebView + explicit actions; wide desktop pages initially fit the available phone width, retain pinch zoom and keep primary form fields/actions reachable without horizontal panning. A matching encrypted verified session is rehydrated with its exact captured user agent before the first load, so re-entry preserves website login while the session remains valid. A Source Home, Search, Detail, Directory or Chapter failure may explicitly enter verified-page mode, which adds operation-specific `打开对应页面` and `使用当前页面`: the first action visibly loads the exact paused GET, preserves provenance only across allowed-origin HTTPS top-frame redirects, and invalidates that binding after any later non-redirect navigation; the second captures only the bound current top frame and returns parser-validated normalized output to the suspended route. Generic completion stores the session but does not claim that the rejected operation recovered. | Internal page scroll | Host feedback constitutional; host chrome remains visually separate from site content. Snapshot bytes remain bounded, memory-only and non-durable; an exact current URL may be used directly, while a redirect final page is accepted only when it remains bound to the explicitly opened paused GET. |
