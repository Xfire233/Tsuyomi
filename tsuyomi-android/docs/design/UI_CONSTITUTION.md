...
| `G-08` | Fixture approval freezes only the reviewed visual contract. Production work still requires separate explicit Gate 4 authorization. |
...
| B · Detail | Header places the fixed five-star local rating to the right of the cover, directly below reading progress. The tag region remains full-width: its left FlowRow takes all space remaining before a content-width `稍后再读` button fixed to the trailing edge. Tags wrap to at most two rows. Every tag, the always-complete tonal `添加标签` action and `稍后再读` use one shared group geometry: `labelLarge`, 40dp visible height, 8dp horizontal content inset, centered 48dp layout/touch slots and `shapes.small`. Their visible border top/bottom coordinates and corner radii match exactly; clickable controls retain 48dp minimum interactive targets. Available tags never collapse into `+n`. The full-width region keeps 4dp external vertical insets. Cache is top-level. Chapter filter/sort/jump actions form one left-aligned cluster. | Leading/following read-later button instead of trailing alignment; fixed-width controls; typography, inner-inset, visible height, slot alignment or corner-radius mismatch inside the group; excessive text-to-border padding; reduced clickable target; more than two tag rows; `+n` hiding fitting tag labels; clipped add-tag label; content-hugging outer region instead of full width; rating below the header or inside the tag region. |
...
- **"No new modules"** means no additional UI-only modules beyond Gate 4's already-planned `shared:library-domain` / `core:library` / `core:media`, plus the `core:preferences` extraction (a re-homing of existing preference code, not a new UI module). The temporary `:prototype:ui-atlas` is non-production and is deleted after approval/extraction (§16.3).

### 2.2 Module DAG (binding; Gate 4 target, enumerated per module)
...
The current Atlas and Gate 4A expose only the bounded query, trailing submit action, layout/sort and one aggregate progress indicator. D33 public/local/source-specific filter descriptors, advanced filter UI and `search-v2` capability negotiation are deferred and impose no Gate 4A implementation dependency.
...
Shared `MutationUiState` + `MutationResultBanner` in `core:ui` (GATE_4 mandated). `idle → working → success | error | cancelled | unresolved`.
...
1. **Full-bleed content:** reading body auto-hides chrome; a center tap, keyboard action or Back reveals chrome without opening a modal. The default surface shows body text and minimal semantic progress; E-ink retains an always-readable chapter/page status.
2. **Visible chrome path:** top bar exposes Back, chapter title, contents/search/bookmark and `阅读设置`. The bottom bar directly exposes previous chapter, chapter-marked progress rail, current location and next chapter. Tap zones and hardware keys remain shortcuts, never the only path.
3. **Progress seek session:** tapping the progress rail immediately commits that semantic target. Dragging/holding creates a transient preview session whose pointer updates never move or persist the live reader; release commits exactly one valid semantic target, cancel restores the opening locator, and `返回原位置` remains available. The exact Standard and E-ink preview presentation is not approved by emulator evidence and is decided only through later physical-device Reader testing.
4. **Stable Reader palette and typography:** Reader foreground/background do not inherit dynamic color. System CJK sans is default; complete typography settings remain available through the advanced page.
5. **Unified auxiliary container:** directory, search, bookmarks and annotations share one Reader-owned auxiliary contract; closing restores locator and focus.
...
- **Standard:** one real M3 `ModalBottomSheet` with partial and full-height anchors. It opens partially; upward drag snaps full. From partial, downward drag, scrim or Back closes. From full, downward drag closes directly rather than stepping back to partial. A drag handle is always visible; there is no title/close row. **E-ink:** the same state/order in an opaque full-window surface without scrim, transparency or animation.
- Quick level omits scope and fully shows four compact rows in the partial viewport: font size, line spacing, margin preset and paragraph spacing. Each row keeps label, current value and slider/step control together. Changes preview and persist immediately; theme, paging/navigation and all remaining controls stay reachable in the same container.
- Complete settings replace content inside the same container and group `排版 / 页面 / 导航 / 设备`. Scope (`全局 / 系列 / 本书`) belongs here as an advanced edit-target setting.
...
- Back: complete-settings subgroup → quick level; at quick level Back closes settings → next Back hides chrome → next Back pops Reader. Drag/scrim closing follows the direct rules above. Closing restores focus and preserves semantic locator.
...
2. **Prototype atlas:** before any Gate 4 production UI implementation, revise the standalone `:prototype:ui-atlas` per `UI_ATLAS.md` RC2.1. The old RC2 renders are rejected evidence, not a baseline to patch incrementally.
...
| UI-P0-B | The mandatory prototype-atlas + manual-approval + separate implementation-authorization gate (§16.3) is bypassed: any Gate 4 production UI change or clean prototype extraction begins before both approvals are recorded. |
...
| UI-P1-0b | Silent mutations: tags/rating/add/remove/pull lack working/success/error/cancelled/unresolved states, duplicate-submit protection, live regions (= G4-UX-003 class) |
| UI-P1-0c | Duplicate detail surfaces + imprecise cross-root action (= G4-UX-001); constitution §13 canonical-detail cutover is the fix |
...
| UI-P1-2 | Production lacks the RC2.1 mixed Library node flow and collection membership UI (= G4-UX-002); remains blocked behind Atlas approval. |
| UI-P1-3 | RECENT/sort/page identity require RC2.1 context keys; E-ink page state must include node/context/layout (= G4-UX-004). |
| UI-P1-4 | Production lacks selection mode and RC2.1 Detail/Reader/Search surfaces; fontScale 2.0 evidence remains required (= G4-UX-005). |
...
| UI-P1-7 | Remote-library: generic route (no sourceId), Boolean loading, no pull taxonomy/count/cancel (= G4-UX-007) |
| UI-P1-8 | Missing routes/surfaces: reader settings, data/report, help and history (= G4-UX-006/008); current collection/template manager must be replaced by the mixed-flow create/rebuild model; single-picker transfer |
...
| `book/{sourceId}/{remoteBookId}?origin=…` | caller's | DT | Book title / Up→caller / cache, overflow | Header → progress + fixed local five-star rating at cover right → tag/dynamic split module → intro → chapter header → full chapters; one direction-driven Continue/top/bottom SmallExtendedFAB; filter funnel/sort arrows/jump | Scroll / E-ink bounded paging as required | No repeated source label or teaching copy; source-specific actions stay inside the compact dynamic module |
| `book/…/reader/{chapterId}` | caller's | RD | Center-tap chrome / Up→detail / contents/search/bookmark/settings | Bottom progress rail with chapter ticks; tap-to-jump; semantic seek commits once; Standard partial/full M3 settings sheet; E-ink dedicated full-screen AppBar settings page with complete `排版 / 页面 / 导航 / 设备` sections, two columns wide and stacked compact | E-ink persistent status; seek-preview presentation requires physical-device approval | Semantic locator only; no current emulator preview approval; Reader-only immersion |
| `search?origin={library|browse}&selectedSourceId={sourceId?}` | caller's | CL | `聚合搜索` / Up / layout | Query draft + one trailing Search action starts local work and the implicit active-source scope together; normal entry covers all active capable sources, source-card entry binds one source; no selector or leading Search control; one aggregate progress; unified exact-identity result flow; no advanced filters in Gate 4A; three layouts | Scroll / pages | No result update/network before submit; no persistent teaching, identity or normal per-source status prose |
...
| 11 | Deferred D33 advanced filtering from both Atlas and Gate 4A; retained one-submit concurrent basic search, one result flow, aggregate progress and exact-identity dedupe. | Explicit conflict decisions |
