...
The Library-root shortcut shelf is **unlocked by default** and participates in the same vertical scroll domain as the root book surface. While unlocked, downward content scroll carries the full shelf upward with its original position; once its lower edge leaves the viewport it resolves to one compact top-edge Material chevron affordance. Activating that chevron, or reversing into an upward scroll gesture, expands the shelf at the preserved Library position. Locking the shelf pins the complete shelf below the fixed app bar exactly as the current Atlas presentation does and removes the compact chevron. Unlocking while the user is already partway through the Library does not jump the book grid: the shelf rejoins the scroll domain and collapses after the next downward scroll takes it beyond the viewport. Standard uses official Compose visibility/size primitives with the centralized 200–250ms top-anchored expand token; interruption retargets without queueing. E-ink and reduced-motion replace immediately. The affordance uses the verified Material expand/collapse symbol, a ≥48dp target and explicit expanded state semantics.
...
The current `phase4-standard-first` execution policy applies two independent deferrals: E-ink remains profile-frozen, and `S*`/`X*` nodes remain stage-deferred to actual online production scenarios. The Prototype row below currently executes the 18 Standard `L*`/`B*`/`M*` nodes; the full production verification matrix remains binding after implementation authorization.
