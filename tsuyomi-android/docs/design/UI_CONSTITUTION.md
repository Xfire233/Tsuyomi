...
| `L042-M5` | Remote ADD and MOVE retain separate default-off per-operation JIT authorization receipts. The first invocation explains and grants that operation; later exact single-book drag/menu invocations may execute without another modal confirmation, but every invocation still requires a visible direct action, a fresh host-minted single-use token, fresh policy/package/credential checks and a durable reconciliation row before transport. MOVE admission and its receipt UI are reachable only after website grouping is enabled. Remote REMOVE is excluded from persistent no-confirm execution: every invocation requires a verb-specific destructive confirmation and leaves local data untouched. This supersedes prior universal per-invocation ADD/MOVE confirmation wording. |
| `L042-M6` | In either mode, dragging a remote book to the transient `复制到本地书架` target copies locally and retains the website item, while the distinct destructive website-removal target requests REMOVE and opens its required confirmation. Grouped mode additionally allows a remote book to be dragged to another remote folder to request MOVE. These targets use different labels, semantics and visual roles. Remote writes remain single-book; local copies and local organization may be batched. |
...
| `B042-B3` | **Direct mirror-action correction.** A website-mirror root never uses an icon-only `+` to mean `固定到快捷书架`, nor an icon-only trash can to mean `移出快捷书架`; those symbols are too easily misread as adding/removing books. Mirror shortcut management is a labelled Material overflow action with the standard pin icon. `全部复制`/`复制所选到本地` uses the distinct copy icon; grouped-only `移至网站分类` uses move-to-folder; destructive `从网站移除` uses delete plus the complete error-coloured verb and still requires confirmation. Direct AppBar icons remain reserved for universally recognizable refresh and layout controls. |
