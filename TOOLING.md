...
| Gradle Wrapper | Affected Android compile/lint/test/build targets selected by a change-to-evidence map and active-profile policy | The smallest affected proof passes; a required full class/suite runs at most once after its group is stable, and the CI-owned matrix is not duplicated locally |
...
| Gradle Wrapper | Repository-owned wrapper | Android build, compiler, lint, unit/instrumentation or screenshot test | Affected tasks, exact reproduction, active profiles and CI-owned claims are known | Run `exact test → adjacent sequence → affected group → one required final suite`; return to the smallest seam after any late failure | Exact bounded build/test result | Repeated full-suite debugging, deferred-profile routine tests, local duplication of the CI matrix, or production changes made only to satisfy idling | CI for the complete planner matrix or an unavailable local environment; reproduce only the exact CI failure | Run the smallest affected compile/test task and verify the selected profile before any broader command |
