...
|---|---|---|---|
| `LOW` | Default when no mode is stated. Also selected by `低占用模式`, `低资源模式`, or an equivalent request, and remains active for the conversation until changed. | Invoke `tsuyomi-android/tools/Run-Gradle-Low.bat` for bounded direct development compilation; it uses `--max-workers=2 --no-parallel --no-daemon`, retaining build-cache, configuration-cache, and incremental compilation while avoiding a persistent Gradle daemon from the invocation. | No local API 29 CI, matrix, preflight, or `--prepare-only`. Hosted protected checks are the only CI path. |
| `HIGH` | Selected only by `高速模式`, `全速模式`, or an equivalent explicit request, and remains active for the conversation until changed. | Invoke `tsuyomi-android/tools/Run-Gradle-High.bat`; it uses `--parallel` and sets `--max-workers` to the machine's logical processor count. Combine compatible Gradle tasks in one invocation instead of starting competing daemons/builds. | After bounded diagnostics, run the local `tools/android_api29.py --mode high --build` gate before hosted checks. Parallelize genuinely independent non-device operations only; its disposable device execution remains serialized. |

Local Android automation invokes `tools/android_api29.py --mode high`; hosted automation invokes the common runner with `--mode ci` only when `GITHUB_ACTIONS=true`. The local runner rejects default/explicit `low` before touching SDK/AVD/Gradle, and rejects non-`HEAD` local `--head`; it records its tracked-dirty/unignored-untracked worktree overlay. `HIGH` omits `--no-daemon` for precompile and direct serialized instrumentation, allowing Gradle to reuse its daemon between phases; hosted `ci` retains `--no-daemon`. The runner calls the repository Gradle Wrapper with their configured limits, delegates selection to `tools/android_ci_plan.py`, loads `tools/android_api29_profile.json`, creates only its disposable automation AVD, records environment/log/phase-timing evidence, and cleans its own resources. `--task`/`--test-class` are focused diagnostics, not a gate, and `--prepare-only` never substitutes for instrumentation. Compare performance only at equal resolved head/overlay policy, scope, selected tasks, profile/image revision and host evidence; no comparison claims Windows/Linux/hosted execution parity. Invocation and Windows/WSL limits are owned by [`tsuyomi-android/README.md`](tsuyomi-android/README.md); visual/human device ownership remains with [`tsuyomi-android/.agents/skills/tsuyomi-android-review/SKILL.md`](.agents/skills/tsuyomi-android-review/SKILL.md).
...
| Gradle Wrapper | Affected Android compile/lint/test/build targets selected by a change-to-evidence map, active-profile policy, and current `LOW`/`HIGH` resource mode | The smallest affected proof passes; in explicit `HIGH`, the full planner-selected local gate completes once before push and hosted protected checks independently complete; in `LOW`, CI completes hosted only; no competing local Gradle invocation remains |
| `tools/android_api29.py` | Disposable API 29 automation runner for explicit-HIGH local preflight and the hosted Android lane | Its local `--mode high --build` gate completes with environment/log evidence retained under `tsuyomi-android/build/api29-ci/` and its own AVD/emulator cleaned; it is never invoked locally in `LOW`, while hosted checks still complete independently |
...
| `tsuyomi-context-router` | Unresolved task ownership and bounded repository context selection | Every requested facet has an owner, affected section, downstream action and authorization boundary; continue the user task without a routing-only yield |
...
| Gradle Wrapper | Repository-owned wrapper plus `tsuyomi-android/tools/Run-Gradle-Low.bat` and `Run-Gradle-High.bat` for direct interactive local mode selection | Android build, compiler, lint, unit/instrumentation or screenshot test | Affected tasks, exact reproduction, active profiles and current `LOW`/`HIGH` mode are known | In `HIGH`, run `exact test → adjacent sequence → local planner-selected gate → hosted protected checks`; in `LOW`, direct compilation is not CI and protected hosted checks are the CI path. `tools/android_api29.py` invokes the wrapper for the local/CI automated gate | Exact bounded build/test result with the selected resource mode visible in output | Repeated full-matrix debugging, local API 29 CI in `LOW`, deferred-profile routine tests, competing Gradle invocations, or production changes made only to satisfy idling | `tools/android_api29.py --mode high --build` for the explicit-HIGH local planner-selected gate; CI is independently required final acceptance | Run the smallest affected compile/test task through the selected batch runner and verify its mode banner plus selected profile before broader diagnostics |
| `tools/android_api29.py` | Repository root runner plus `tools/android_api29_profile.json` | Explicit-HIGH local Android preflight or the hosted Android lane | SDK path (`ANDROID_SDK_ROOT`/`ANDROID_HOME` or `--sdk`) and base when available are known; local invocation explicitly uses `--mode high` and `--head HEAD` only; `--mode ci` requires `GITHUB_ACTIONS=true`; omit an invalid/unavailable base for the planner's conservative full fallback; no canonical or Review_Work device is selected | Every non-prepare run precompiles selected instrumentation `:module:assembleDebug` and `:module:assembleDebugAndroidTest`; `HIGH` omits `--no-daemon` for that work and serialized instrumentation, allowing daemon reuse between phases, while `ci` retains `--no-daemon`; add non-focused `--build` for the local full gate and its planner-selected build/lint/JVM/screenshot tasks, use `--task` plus exact `--test-class` only for a focused diagnostic, or `--prepare-only` for Gradle-free lifecycle evidence | Disposable `tsuyomi-ci-*` AVD plus `environment.json` recording run/mode/scope/full-gate status, planner base/head/fallback/reasons, resolved head/worktree overlay, resolved profile/AVD, host, SDK revisions, tools, selected tasks/logs, `timings_seconds.precompile`, `.emulator_prepare`, `.instrumentation` (absent for prepare-only), `.total` and exit/failure; supplemental device/log artifacts live under `tsuyomi-android/build/api29-ci/<run>/` | Any local invocation in `LOW` (including `--prepare-only`), a local non-`HEAD` run, visual/human review, canonical device/APK replacement, approval, manual duplicate AVD configuration, or a focused diagnostic presented as the gate | If the fixed system image is absent, stop and follow the runner's actionable SDK install instruction; it never installs packages or accepts licenses | In explicit `HIGH`, `--prepare-only` creates, configures, records and cleans only the runner-owned AVD |
...
Context-router adoption, material routing expansion, and memory-maintenance extension enablement require a predeclared historical-task replay with equal task scope, frozen inputs, model/settings and answer-quality criteria. Include candidate instructions, retained notices and ordinary-task overhead in reported token usage, count cached input as tokens, and declare the timing boundary. Both elapsed time and total tokens must decrease without lost required evidence; failure of either metric, missing usage or unequal quality means do not adopt the candidate. Record sample size, cache conditions and limitations with local evidence; a single-model pilot never proves universal savings. This gate governs changes, not ordinary Skill calls.
...
| `tsuyomi-context-router` | Project: `.agents/skills/tsuyomi-context-router` | WORKSPACE context-routing entry matches; Skill owns detailed trigger/exclusion algorithm | Read Skill and reuse current bootstrap/context; private memory is optional navigation input | Select existing owners and complete relevant sections, then release to the specialist/tool | Minimal in-session owner/section/action mapping; no persistent index | Known-owner local tasks, specialist re-entry, product rules, memory writes, permission grants or duplicate registries | Direct canonical file read when discovery is absent; existing DOCUMENTATION dispatch if file is unavailable, recording the defect | Fresh-session registry plus canonical Skill read; bounded invocation/bypass replay |
