...
Interactive local work has two workstation scheduling modes. The selected mode changes concurrency and resource pressure only; it never removes required evidence, changes the verification ladder, authorizes a broader scope, or permits simultaneous Gradle invocations against the same checkout.

| Mode | Trigger and persistence | Gradle execution | Other local execution |
|---|---|---|---|
| `LOW` | Default when no mode is stated. Also selected by `低占用模式`, `低资源模式`, or an equivalent request, and remains active for the conversation until changed. | Invoke `tsuyomi-android/tools/Run-Gradle-Low.bat`; it uses `--max-workers=2 --no-parallel --no-daemon`, retaining build-cache, configuration-cache, and incremental compilation while avoiding a persistent Gradle daemon from the invocation. | Run CPU-, memory-, disk-, emulator-, and network-heavy operations sequentially. Stop unneeded AVDs before Gradle/lint, do not overlap repository scanners with a build, and parallelize only negligible read-only lookups. |
| `HIGH` | Selected only by `高速模式`, `全速模式`, or an equivalent explicit request, and remains active for the conversation until changed. | Invoke `tsuyomi-android/tools/Run-Gradle-High.bat`; it uses `--parallel` and sets `--max-workers` to the machine's logical processor count. Combine compatible Gradle tasks in one invocation instead of starting competing daemons/builds. | Parallelize genuinely independent operations and keep required runtime resources available when overlap shortens the critical path. Safety, device ownership, bounded verification, and cleanup rules remain unchanged. |

CI and external automation continue to call the repository Gradle Wrapper directly and own their configured worker limits. The two batch runners own local interactive selection because command-line flags override the repository's throughput-oriented `org.gradle.parallel=true` without changing shared Gradle defaults. Record the current mode in `.local/ACTIVE_HANDOFF.md` for cross-turn continuity.
...
| Gradle Wrapper | Affected Android compile/lint/test/build targets selected by a change-to-evidence map, active-profile policy, and current `LOW`/`HIGH` resource mode | The smallest affected proof passes; a required full class/suite runs at most once after its group is stable, the CI-owned matrix is not duplicated locally, and no competing local Gradle invocation remains |
...
| Gradle Wrapper | Repository-owned wrapper plus `tsuyomi-android/tools/Run-Gradle-Low.bat` and `Run-Gradle-High.bat` for interactive local mode selection | Android build, compiler, lint, unit/instrumentation or screenshot test | Affected tasks, exact reproduction, active profiles, CI-owned claims, and current `LOW`/`HIGH` mode are known | Run `exact test → adjacent sequence → affected group → one required final suite`; local interactive commands use the selected batch runner, while CI calls the wrapper directly | Exact bounded build/test result with the selected resource mode visible in output | Repeated full-suite debugging, deferred-profile routine tests, local duplication of the CI matrix, competing Gradle invocations, or production changes made only to satisfy idling | CI for the complete planner matrix or an unavailable local environment; reproduce only the exact CI failure | Run the smallest affected compile/test task through the selected batch runner and verify its mode banner plus selected profile before any broader command |
