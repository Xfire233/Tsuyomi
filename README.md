...
### [Unreleased] (Phase 4B Website Library)
- **网站收藏镜像**：根/分组镜像、快捷栏固定、Library 原生布局与显式前台刷新。
- **安全回写**：单书 `ADD`/`MOVE`/`REMOVE` 独立授权、独立目标发现、持久化对账、取消/未决/部分成功恢复。
- **详情与阅读**：本地加入和网站收藏分流、菜单外持久反馈、精确章节完成、离线优先详情与显式缓存恢复。
- **迁移与质量**：Room v5–v8、transfer v2、生产 Review Graph、按 merge-base 选择的 CI 任务和退役原型清理。
