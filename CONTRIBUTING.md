...
- Android UI、导航、交互、原型、显示配置或无障碍改动先按 `.agents/skills/tsuyomi-android-review/SKILL.md` 执行 `UI-R1`；当前活跃/延后 profile 只读取相邻的 `review-policy.json`。`.local/` 影响报告只选择节点和验证层，不表示批准。
