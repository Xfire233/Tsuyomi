...
- Strict `tsuyomi-transfer` v4 schema, fixtures, and conformance coverage add portable chapter bookmarks plus full global Reader typography and color preferences. V4 requires explicit bookmark arrays, retains unpinned records without creating a pin, and keeps all v1/v2/v3 schemas closed to v4-only fields.
- Strict `tsuyomi-transfer` v5 schema, semantic validator coverage, and fixtures replace chapter bookmark IDs with bounded `ReaderLocator` bookmarks. V5 requires locator documents to belong to their enclosing books, rejects duplicate exact semantic positions regardless of capture timestamps or rendered metrics, preserves non-pinned metadata, and keeps v1–v4 closed input contracts unchanged.
